    ...
    static async fetchTranscript(videoId: string, preferredLang: string = 'Auto'): Promise<SubtitleCue[]> {
        try {
            ...
            if (preferredLang !== 'Auto') {
                const matchesPref = (t: any) => t.name?.simpleText?.toLowerCase().includes(lowerPref) || t.languageCode === lowerPref.substring(0, 2);
                track = captionTracks.find((t: any) => matchesPref(t) && t.kind !== 'asr' && !t.vssId?.startsWith('a.'));
                if (!track) track = captionTracks.find((t: any) => matchesPref(t));
            }

            // Fallback to any manual track, then default track
            if (!track) track = captionTracks.find((t: any) => t.kind !== 'asr' && !t.vssId?.startsWith('a.'));

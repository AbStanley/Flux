...
- Provide explanations in ${"{nativeLang}"} when introducing new concepts.
- Focus on present tense and basic sentence structures.`,
  intermediate: `- Use everyday vocabulary with some variety. Sentences can be moderate length.
- Include occasional idioms or colloquial expressions, explaining them.
- Use a mix of tenses (present, past, future).
- Challenge the user gently with slightly above their comfort zone.`,
  advanced: `- Speak naturally as you would to a native speaker — full speed, complex grammar, idioms.
- Use subjunctive, conditional, compound tenses, and nuanced expressions freely.
- Do not simplify your language. Only correct actual mistakes, not style preferences.
- Discuss abstract topics and expect detailed responses.`,
};
...
function buildSystemPrompt(
  ...
  return `You are a strict but friendly ${targetLang} language tutor in a conversation practice session. The student's level is ${level.toUpperCase()}. Follow these rules EXACTLY:

LEVEL — ADAPT YOUR LANGUAGE:
${levelBlock}
...
- ONLY correct the user's messages. NEVER correct your own text. On the first message (the greeting), there is nothing to correct.
- For EACH user mistake, output EXACTLY this format on its own line BEFORE your conversational reply:
...
- If the word is a verb, ALWAYS use the infinitive form (e.g. "necesitar — to need", not "necesitas — need").
...
export const useConversationStore = create<ConversationState>()(
  persist(
    (set, get) => ({
      ...
      startConversation: () => {
        const { targetLanguage, nativeLanguage, topic, level } = get();
